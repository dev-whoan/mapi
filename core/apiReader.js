  ...
  readConfigs() {
    ...
    configsInApi.forEach((file) => {
      ...
      if (oneObject.data.uri.includes("@") || oneObject.data.id.includes("@")) {
        ...
        console.warn(
          `The configId ${oneObject.data.uri} + '@' + ${oneObject.data.id} will not be registered.`
        );
        return;
      }
      ...
      if (this.configInfo.get(configId)) {
        console.warn(
          `API Config is duplicated. The new config ${configId} will be set.`
        );
        console.warn(
          `To prevent API Config duplication, please set the concatenation of uri and id into unique string.`
        );
      }
